...
/*
 * Optional transport supplied by an embedding application.  The callbacks
 * operate on complete SRT datagrams (protocol header followed by payload),
 * while libsrt continues to own packet construction, timers and reliability.
 * The structure is copied by srt_set_external_transport(); the opaque value
 * must remain valid until srt_close() has completed.
 */
#define SRT_TRANSPORT_V1_VERSION 1u
typedef struct SRT_TRANSPORT_V1 {
   uint32_t version;
   uint32_t size;
   void *opaque;
   int (*send_datagram)(void *opaque,
                        const uint8_t *header, size_t header_len,
                        const uint8_t *payload, size_t payload_len,
                        const struct sockaddr_storage *destination);
   int (*receive_datagram)(void *opaque,
                           uint8_t *buffer, size_t capacity, size_t *received,
                           struct sockaddr_storage *source, int timeout_ms);
...
SRT_API       int srt_set_external_transport(SRTSOCKET u, const SRT_TRANSPORT_V1 *transport);
